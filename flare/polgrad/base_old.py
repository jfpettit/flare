    ...
    def learn(
        ...
        for i in range(epochs):
            ...
            for j in range(self.steps_per_epoch):
                ...
                action, _, logp, value = self.ac(torch.as_tensor(state, dtype=torch.float32))
                ...
                if over or (j == self.steps_per_epoch - 1):
                    ...
                    if (episode_length == horizon) or (_ == self.steps_per_epoch - 1):
                        last_val = (
                            self.ac.value_f(torch.as_tensor(state)).item()
                        )
                    else:
                        last_val = 0    
